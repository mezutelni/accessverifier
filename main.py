...
def fetch_allowed_ips(list_endpoint, allowed_region):
    ...
    while True:
        try:
            ...
            logger.critical(f"Couldn't fetch allowed IP list\nRequest error - {e}")
            sys.exit(1)
        try:
            response_json = response.json()
            logger.debug(f"Started processing prefixes from {list_endpoint} allowed region {allowed_region}")
            for block in response_json["prefixes"]:
                if allowed_region in block["region"].lower():
                    allowed_ips.append(block["ip_prefix"])
                    logger.debug(f"Added {block['ip_prefix']} to allowed IPs list")
...
if __name__ == "__main__":
    ...
    background_thread = threading.Thread(target=fetch_allowed_ips, args=(Config.allowed_ip_list_endpoint, Config.allowed_region), daemon=True)
